...
print STDERR "Using tmp directory: $tmp\n";
...
# test 7
@args = ( "-t", "species", "-f", "embl", "-a", "not/a/real/dest" );
$obj = Path::Find::CommandLine::Ref->new(args => \@args, script_name => $script_name);
throws_ok {$obj->run} 'Path::Find::Exception::InvalidInput', 'correct error thrown; test #7';

# test 8
@args = ( "-t", "species", "-f", "annotation", "-l", "$tmp/valid_dest" );
$obj = Path::Find::CommandLine::Ref->new(args => \@args, script_name => $script_name);
...
# test 10
@args = ( "-t", "species", "-i", "pseudo", "-f", "fa", "-a", "$tmp/valid_dest" );
...
# test 11
@args = ( "-t", "species", "-i", "pseudo", "-f", "fa", "-a", "/not/a/real/dest" );
$obj = Path::Find::CommandLine::Ref->new(args => \@args, script_name => $script_name);
throws_ok {$obj->run} 'Path::Find::Exception::InvalidDestination', 'correct error thrown; test #11';

# test 12
@args = ( "-t", "species", "-i", "pseudo", "-f", "fa", "-l", "$tmp/valid_dest" );
...
# test 13
@args = ( "-t", "species", "-i", "pseudo", "-f", "fa", "-l", "not/a/real/dest" );
$obj = Path::Find::CommandLine::Ref->new(args => \@args, script_name => $script_name);
...
# test 15
@args = ( "-t", "species", "-i", "pseudo", "-f", "gff", "-a", "$tmp/valid_dest" );
...
# test 16
@args = ( "-t", "species", "-i", "pseudo", "-f", "gff", "-a", "not/a/real/dest" );
$obj = Path::Find::CommandLine::Ref->new(args => \@args, script_name => $script_name);
throws_ok {$obj->run} 'Path::Find::Exception::InvalidDestination', 'correct error thrown; test #16';

# test 17
@args = ( "-t", "species", "-i", "pseudo", "-f", "gff", "-l", "$tmp/valid_dest" );
...
# test 18
@args = ( "-t", "species", "-i", "pseudo", "-f", "gff", "-l", "not/a/real/dest" );
$obj = Path::Find::CommandLine::Ref->new(args => \@args, script_name => $script_name);
...
# test 20
@args = ( "-t", "species", "-i", "pseudo", "-f", "embl", "-a", "$tmp/valid_dest" );
...
# test 21
@args = ( "-t", "species", "-i", "pseudo", "-f", "embl", "-a", "not/a/real/dest" );
$obj = Path::Find::CommandLine::Ref->new(args => \@args, script_name => $script_name);
throws_ok {$obj->run} 'Path::Find::Exception::InvalidDestination', 'correct error thrown; test #21';

# test 22
@args = ( "-t", "species", "-i", "pseudo", "-f", "embl", "-l", "$tmp/valid_dest" );
...
# test 23
@args = ( "-t", "species", "-i", "pseudo", "-f", "embl", "-l", "not/a/real/dest" );
$obj = Path::Find::CommandLine::Ref->new(args => \@args, script_name => $script_name);
...
# test 25
@args = ( "-t", "species", "-i", "pseudo", "-f", "annotation", "-a", "$tmp/valid_dest" );
...
# test 26
@args = ( "-t", "species", "-i", "pseudo", "-f", "annotation", "-a", "not/a/real/dest" );
$obj = Path::Find::CommandLine::Ref->new(args => \@args, script_name => $script_name);
throws_ok {$obj->run} 'Path::Find::Exception::InvalidDestination', 'correct error thrown; test #26';

# test 27
@args = ( "-t", "species", "-i", "pseudo", "-f", "annotation", "-l", "$tmp/valid_dest" );
...
# test 32
@args = ( "-t", "species", "-i", "invalid_value", "-f", "annotation", "-l", "$tmp/valid_dest" );
$obj = Path::Find::CommandLine::Ref->new(args => \@args, script_name => $script_name);
throws_ok {$obj->run} 'Path::Find::Exception::NoMatches', 'correct error thrown; test #32';

# test 33
@args = ( "-t", "file", "-f", "fa", "-l", "not/a/real/dest" );
$obj = Path::Find::CommandLine::Ref->new(args => \@args, script_name => $script_name);
...
# test 38
@args = ( "-t", "file", "-i", "t/data/reffind/ref_lanes.txt", "-f", "fa", "-a", "$tmp/valid_dest" );
...
# test 39
@args = ( "-t", "file", "-i", "t/data/reffind/ref_lanes.txt", "-f", "fa", "-a", "not/a/real/dest" );
$obj = Path::Find::CommandLine::Ref->new(args => \@args, script_name => $script_name);
throws_ok {$obj->run} 'Path::Find::Exception::InvalidDestination', 'correct error thrown; test #39';

# test 40
@args = ( "-t", "file", "-i", "t/data/reffind/ref_lanes.txt", "-f", "fa", "-l", "$tmp/valid_dest" );
$exp_out = read_file("t/data/reffind/40.txt");
$obj = Path::Find::CommandLine::Ref->new(args => \@args, script_name => $script_name);
$arg_str = join(" ", @args);
stdout_is { $obj->run } $exp_out, "Correct results for '$arg_str'";

ok( -e "$tmp/$tmp/valid_dest", 'symlink dir exists' );
ok( check_links('valid_dest', $exp_out), 'correct files symlinked' );

# test 41
@args = ( "-t", "file", "-i", "t/data/reffind/ref_lanes.txt", "-f", "fa", "-l", "not/a/real/dest" );
$obj = Path::Find::CommandLine::Ref->new(args => \@args, script_name => $script_name);
...
# test 43
@args = ( "-t", "file", "-i", "t/data/reffind/ref_lanes.txt", "-f", "gff", "-a", "$tmp/valid_dest" );
...
# test 44
@args = ( "-t", "file", "-i", "t/data/reffind/ref_lanes.txt", "-f", "gff", "-a", "not/a/real/dest" );
$obj = Path::Find::CommandLine::Ref->new(args => \@args, script_name => $script_name);
throws_ok {$obj->run} 'Path::Find::Exception::InvalidDestination', 'correct error thrown; test #44';

# test 45
@args = ( "-t", "file", "-i", "t/data/reffind/ref_lanes.txt", "-f", "gff", "-l", "$tmp/valid_dest" );
...
# test 46
@args = ( "-t", "file", "-i", "t/data/reffind/ref_lanes.txt", "-f", "gff", "-l", "not/a/real/dest" );
$obj = Path::Find::CommandLine::Ref->new(args => \@args, script_name => $script_name);
...
# test 48
@args = ( "-t", "file", "-i", "t/data/reffind/ref_lanes.txt", "-f", "embl", "-a", "$tmp/valid_dest" );
...
# test 49
@args = ( "-t", "file", "-i", "t/data/reffind/ref_lanes.txt", "-f", "embl", "-a", "not/a/real/dest" );
$obj = Path::Find::CommandLine::Ref->new(args => \@args, script_name => $script_name);
throws_ok {$obj->run} 'Path::Find::Exception::InvalidDestination', 'correct error thrown; test #49';

# test 50
@args = ( "-t", "file", "-i", "t/data/reffind/ref_lanes.txt", "-f", "embl", "-l", "$tmp/valid_dest" );
...
# test 51
@args = ( "-t", "file", "-i", "t/data/reffind/ref_lanes.txt", "-f", "embl", "-l", "not/a/real/dest" );
$obj = Path::Find::CommandLine::Ref->new(args => \@args, script_name => $script_name);
...
# test 53
@args = ( "-t", "file", "-i", "t/data/reffind/ref_lanes.txt", "-f", "annotation", "-a", "$tmp/valid_dest" );
...
# test 54
@args = ( "-t", "file", "-i", "t/data/reffind/ref_lanes.txt", "-f", "annotation", "-a", "not/a/real/dest" );
$obj = Path::Find::CommandLine::Ref->new(args => \@args, script_name => $script_name);
throws_ok {$obj->run} 'Path::Find::Exception::InvalidDestination', 'correct error thrown; test #54';

# test 55
@args = ( "-t", "file", "-i", "t/data/reffind/ref_lanes.txt", "-f", "annotation", "-l", "$tmp/valid_dest" );
...
# test 56
@args = ( "-t", "file", "-i", "t/data/reffind/ref_lanes.txt", "-f", "annotation", "-l", "not/a/real/dest" );
$obj = Path::Find::CommandLine::Ref->new(args => \@args, script_name => $script_name);
throws_ok {$obj->run} 'Path::Find::Exception::InvalidDestination', 'correct error thrown; test #56';

# test 57
@args = ( "-t", "file", "-i", "invalid_value", "-f", "fa", "-l", "$tmp/valid_dest" );
$obj = Path::Find::CommandLine::Ref->new(args => \@args, script_name => $script_name);
throws_ok {$obj->run} 'Path::Find::Exception::FileDoesNotExist', 'correct error thrown; test #57';

# test 58
@args = ( "-t", "file", "-i", "invalid_value", "-f", "gff", "-a", "$tmp/valid_dest" );
$obj = Path::Find::CommandLine::Ref->new(args => \@args, script_name => $script_name);
...
# test 60
@args = ( "-t", "file", "-i", "invalid_value", "-f", "annotation", "-l", "$tmp/valid_dest" );
$obj = Path::Find::CommandLine::Ref->new(args => \@args, script_name => $script_name);
throws_ok {$obj->run} 'Path::Find::Exception::FileDoesNotExist', 'correct error thrown; test #60';

# test 61
@args = ( "-t", "species", "-i", "pseudo", "-f", "gff", "-l", "$tmp/valid_dest", "-h" );
$obj = Path::Find::CommandLine::Ref->new(args => \@args, script_name => $script_name);
throws_ok {$obj->run} 'Path::Find::Exception::InvalidInput', 'correct error thrown; test #61';

# test 62
@args = ( "-t", "file", "-i", "t/data/reffind/ref_lanes.txt", "-f", "embl", "-l", "$tmp/valid_dest", "-a", "$tmp/valid_dest" );
$obj = Path::Find::CommandLine::Ref->new(args => \@args, script_name => $script_name);
