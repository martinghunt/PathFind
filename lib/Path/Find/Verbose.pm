...
has 'date'      => ( is => 'ro', isa => 'Str',      required => 0 );
has 'found' =>
...
sub _reference_ur {
    ...
    open( SQ, "-|", "samtools view -H $bam_file | grep ^\@SQ" ) or die "$bam_file could not be opened\n";
    while ( my $line = <SQ> ) {
        if ( $line =~ /UR:file:(.+)fa/ ) {
            return $1 . "gff";
        }
...
sub _get_mapper {
    ...
    open( PG, "-|", "samtools view -H $bam_file | grep ^\@PG" ) or die "$bam_file could not be opened\n";
    while ( my $line = <PG> ) {
        if ( $line =~ /PP:([^\s]+)/ ) {
